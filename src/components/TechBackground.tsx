      ...
      for (let i = 0; i < numPaths; i++) {
        ...
        for (let p = 0; p < pulseCount; p++) {
          activeNodes.push({
            ...
            size: Math.random() * 2 + (layer === 3 ? 3 : 1.5)
          });
        }
      ...
      [1, 2, 3].forEach((layer) => {
        paths.forEach((path) => {
          ...
          if (path.layer === 3) {
            ...
            ctx.shadowBlur = 8 * nodeGlowMultiplier;
            ctx.shadowColor = '#00f0ff';
            ctx.globalAlpha = Math.min(0.85 * lineOpacityMultiplier, 1.0);
          } else if (path.layer === 2) {
            ctx.lineWidth = 0.8;
            ctx.strokeStyle = path.color;
            ctx.globalAlpha = Math.min(0.4 * lineOpacityMultiplier, 0.95);
          } else {
            ctx.lineWidth = 0.5;
            ctx.strokeStyle = path.color;
            ctx.globalAlpha = Math.min(0.2 * lineOpacityMultiplier, 0.8);
          }
          ctx.stroke();
          ctx.restore();
          ...
          path.activeNodes.forEach((node) => {
            ...
            ctx.fillStyle = path.layer === 3 ? '#ffffff' : 'rgba(0, 240, 255, 0.85)';
            
            if (path.layer === 3) {
              ctx.shadowBlur = 12 * nodeGlowMultiplier;
              ctx.shadowColor = '#00f0ff';
            }
            ctx.globalAlpha = path.layer === 3 ? 0.95 : path.layer === 2 ? 0.6 : 0.3;
            ctx.fill();
            ctx.restore();
          });
          ...
          if (path.layer === 3) {
            ...
            ctx.shadowBlur = 10 * nodeGlowMultiplier;
            ctx.shadowColor = '#00f0ff';
            ctx.globalAlpha = pulseAlpha;
          } else if (path.layer === 2) {
            ctx.fillStyle = 'rgba(0, 114, 255, 0.7)';
            ctx.globalAlpha = pulseAlpha * 0.6;
          } else {
            ctx.fillStyle = 'rgba(0, 114, 255, 0.3)';
